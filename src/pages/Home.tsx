...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] selection:bg-gold/30 text-white font-sans">
      ...
      <section className="h-screen relative overflow-hidden flex items-center justify-center">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-6xl md:text-9xl font-black mb-8 leading-[0.8] tracking-tighter uppercase italic italic-subtle">
              {t('hero.title')}
            </h1>
            <p className="text-xl md:text-2xl text-zinc-500 mb-10 max-w-3xl mx-auto font-light leading-relaxed">
              {t('hero.subtitle')}
            </p>
            <motion.button
      ...
      <section className="py-40 relative bg-[#111111]">
        <MovingLines />
        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="max-w-4xl mx-auto">
             <Compass className="w-12 h-12 text-zinc-600 mx-auto mb-12 opacity-50" />
             <h2 className="text-3xl md:text-6xl font-extralight leading-tight text-zinc-200">
               {t('about.text')}
             </h2>
             
             <div className="mt-24 grid md:grid-cols-3 gap-16">
                <div className="flex flex-col items-center">
                   <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group hover:border-gold/50 transition-colors">
                      <Globe2 className="w-6 h-6 text-zinc-400 group-hover:text-gold transition-colors" />
                   </div>
                   <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">International</p>
                </div>
                <div className="flex flex-col items-center">
                   <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group hover:border-teal/50 transition-colors">
                      <Shield className="w-6 h-6 text-zinc-400 group-hover:text-teal transition-colors" />
                   </div>
                   <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Stability</p>
                </div>
                <div className="flex flex-col items-center">
                   <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group hover:border-gold/50 transition-colors">
                      <TrendingUp className="w-6 h-6 text-zinc-400 group-hover:text-gold transition-colors" />
                   </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {divisions.map((div, i) => (
              <motion.div key={div.id} whileHover={{ y: -10 }}>
                <Link to={div.link} className="block relative bg-[#141414] p-12 border border-white/5 h-full group hover:border-zinc-700 transition-all duration-500">
                  <div className="relative z-10 text-center">
                    <img src={div.logo} alt={div.id} className="h-20 mx-auto mb-10 opacity-40 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500" />
                    <h3 className="text-2xl font-bold uppercase mb-4 tracking-tighter" style={{ color: div.color }}>{t(`divisions.${div.id}.title`)}</h3>
                    <p className="text-zinc-500 text-sm font-light leading-relaxed mb-8">{t(`divisions.${div.id}.description`)}</p>
      ...
      <section className="py-40 bg-[#111111] border-y border-white/5">
        <div className="container mx-auto px-6">
          <div className="flex flex-col lg:flex-row gap-20 items-end mb-24">
            <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-none flex-1">
          ...
          <div className="grid md:grid-cols-3 gap-8">
             {(t('value.benefits', { returnObjects: true }) as any[]).map((benefit, index) => (
                <div key={index} className="bg-[#1A1A1A] p-12 group border-l border-zinc-800 hover:border-gold transition-all duration-500">
                  <span className="text-3xl font-black text-white/5 block mb-8 group-hover:text-gold/20 transition-colors">0{index + 1}</span>
                  <h3 className="text-lg font-black uppercase mb-4 tracking-widest">{benefit.title}</h3>
            ...
            {(t('services.steps', { returnObjects: true }) as any[]).map((step, index) => (
              ...
              >
                <div className={`relative ${index % 2 !== 0 ? 'md:order-2' : ''}`}>
                   <span className="absolute -top-24 -left-10 text-[180px] font-black text-white/[0.02] select-none">{step.number}</span>
                   <h3 className="text-4xl font-black uppercase italic tracking-tighter mb-6 relative z-10">{step.title}</h3>
                   <p className="text-zinc-500 text-lg font-extralight leading-relaxed italic">{step.description}</p>
                </div>
        ...
        <div className="container mx-auto px-6">
           <div className="grid lg:grid-cols-2 gap-32">
              <div>
                ...
                <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter italic mb-8 italic-subtle">{t('targetAudience.title')}</h2>
                <div className="p-8 border-l-2 border-zinc-700 bg-white/[0.02]">
                   <p className="text-2xl font-light italic text-zinc-400 leading-snug">"{t('targetAudience.subtitle')}"</p>
                </div>
              </div>
              <div className="space-y-4 self-center">
                 {(t('targetAudience.investors', { returnObjects: true }) as string[]).map((investor, idx) => (
                    <motion.div key={idx} whileHover={{ x: 10 }} className="p-6 border border-zinc-800 flex justify-between items-center group cursor-pointer hover:bg-zinc-800/30">
                       <span className="text-sm font-bold uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors">{investor}</span>
      ...
      <section className="py-32 bg-[#0A0A0A]">
        <div className="container mx-auto px-6">
           <h2 className="text-5xl md:text-9xl font-black uppercase tracking-tighter mb-20 italic italic-subtle text-white">Locations</h2>
           <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
             {cities.map((city) => (
               <Link to={`/${city.name}`} key={city.name}>
                  <motion.div className="h-[500px] relative group overflow-hidden border border-white/5 grayscale hover:grayscale-0 transition-all duration-1000">
                     ...
                     <div className="absolute inset-0 p-8 flex flex-col justify-end">
                        ...
                        <h3 className="text-4xl font-black uppercase tracking-tighter mb-4">{city.name}</h3>
                        <div className="w-12 h-12 bg-white flex items-center justify-center -mb-20 group-hover:mb-0 transition-all duration-500">
      ...
      <section className="py-40 bg-[#0A0A0A] border-t border-zinc-900 relative">
         <div className="absolute inset-0 bg-gradient-to-t from-zinc-900/40 to-transparent" />
         <div className="container mx-auto px-6 relative z-10 text-center">
            <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tighter mb-12 max-w-5xl mx-auto italic italic-subtle">{t('cta.title')}</h2>
            <div className="flex flex-col md:flex-row justify-center items-center gap-6">
               <motion.a href="mailto:info@ahox-cm.de" className="w-full md:w-auto px-16 py-6 bg-white text-black font-black uppercase text-xs tracking-widest hover:bg-gold transition-colors shadow-2xl">
                 {t('cta.button')}
               </motion.a>
               <motion.a href="tel:+496927278761" className="w-full md:w-auto px-16 py-6 border border-zinc-700 text-zinc-400 font-black uppercase text-xs tracking-widest hover:border-white hover:text-white transition-all">
